  ...
  if (model.indexSignature || model.children) {
    ...
    if (declarationIndexSignature) {
      const key = declarationIndexSignature.parameters
        ? declarationIndexSignature.parameters.map(
            (param) => `\`[${camelToSnakeCase(param.name)}: ${param.type}]\``
          )
        : "";
    ...
    const types =
      ...
      model.children.map((obj) => {
        return `\`${camelToSnakeCase(obj.name)}${
...
function getReferenceType(model: ReferenceType, emphasis) {
  if (model.reflection || (model.name && model.typeArguments)) {
    ...
    if (model.reflection?.url) {
      reflection.push(
        ...
        )}\``}](${Handlebars.helpers.relativeURL(model.reflection.url)})`
      );
    } else {
      reflection.push(
        model.externalUrl
          ? `[${`\`${camelToSnakeCase(model.name)}\``}]( ${model.externalUrl} )`
  ...
  return emphasis
    ? model.externalUrl
      ? `[${`\`${camelToSnakeCase(model.name)}\``}]( ${model.externalUrl} )`
      : `\`${camelToSnakeCase(model.name)}\``
    : escapeChars(camelToSnakeCase(model.name));
}
...
function getUnknownType(model: UnknownType) {
  return escapeChars(camelToSnakeCase(model.name));
}
