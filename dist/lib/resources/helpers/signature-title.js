...
function default_1(theme) {
    Handlebars.registerHelper("signatureTitle", function (accessor, standalone = true) {
        ...
        if (accessor) {
            md.push(`\`${accessor}\` **${(0, utils_1.camelToSnakeCase)(this.name)}**`);
        }
        else if (this.name !== "__call" && this.name !== "__type") {
            md.push(`**${(0, utils_1.camelToSnakeCase)(this.name)}**`);
        }
...
const getParameters = (parameters = [], backticks = true) => {
    return parameters
        ...
        const paramItem = `${(0, utils_1.camelToSnakeCase)(param.name)}${param.flags.isOptional || param.defaultValue ? "?" : ""}`;
