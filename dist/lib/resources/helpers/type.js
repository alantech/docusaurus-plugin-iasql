    ...
    if (model.indexSignature || model.children) {
        ...
        if (declarationIndexSignature) {
            const key = declarationIndexSignature.parameters
                ? declarationIndexSignature.parameters.map((param) => `\`[${(0, utils_1.camelToSnakeCase)(param.name)}: ${param.type}]\``)
                : "";
        ...
        const types = model.children &&
            model.children.map((obj) => {
                return `\`${(0, utils_1.camelToSnakeCase)(obj.name)}${obj.flags.isOptional ? "?" : ""}\`: ${Handlebars.helpers.type.call(obj.signatures || obj.children ? obj : obj.type)} ${obj.defaultValue && obj.defaultValue !== "..."
...
function getReferenceType(model, emphasis) {
    ...
    if (model.reflection || (model.name && model.typeArguments)) {
        ...
        if ((_a = model.reflection) === null || _a === void 0 ? void 0 : _a.url) {
            reflection.push(`[${`\`${(0, utils_1.camelToSnakeCase)(model.reflection.name)}\``}](${Handlebars.helpers.relativeURL(model.reflection.url)})`);
        }
        else {
            reflection.push(model.externalUrl
                ? `[${`\`${(0, utils_1.camelToSnakeCase)(model.name)}\``}]( ${model.externalUrl} )`
                : `\`${(0, utils_1.camelToSnakeCase)(model.name)}\``);
        }
    ...
    return emphasis
        ? model.externalUrl
            ? `[${`\`${(0, utils_1.camelToSnakeCase)(model.name)}\``}]( ${model.externalUrl} )`
            : `\`${(0, utils_1.camelToSnakeCase)(model.name)}\``
        : (0, utils_1.escapeChars)((0, utils_1.camelToSnakeCase)(model.name));
}
...
function getUnknownType(model) {
    return (0, utils_1.escapeChars)((0, utils_1.camelToSnakeCase)(model.name));
}
